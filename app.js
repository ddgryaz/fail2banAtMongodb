...
async function refreshState(){
  for (var jailIdx=0;jailIdx<jailNames.length;jailIdx++){
    ...
    let newStatus=await jail.status;

    for(var i=0;i<newStatus.actions.bannedIPList.length;i++){
      let ip=newStatus.actions.bannedIPList[i];
...
async function refreshDbIgnores(){
  let docs=await mdb.collection('ignore').find({}).toArray();
  let newIgnore={};
  let ipsList=[];
  for (var i=0;i<docs.length;i++){
    newIgnore[docs[i].ip]=docs[i];
    ipsList.push(docs[i].ip);
  }
  for (var ip in newIgnore){
...
async function refreshDbBans(){
  let unbanDocs=await mdb.collection('unban').find({
    t:{$gt:state.times.unban}
  }).toArray();
  for (var i=0;i<unbanDocs.length;i++){
    let unbanDoc=unbanDocs[i];
    //console.log(`Unban ip:${unbanDoc.ip} receivedFrom:${banDoc.serverName||banDoc.appDb}`);
    unbanIp[unbanDoc.ip]=unbanDoc;
    state.times.unban=new Date(
      Math.max(state.times.unban.getTime(),unbanDoc.t.getTime()));
  }

  let banDocs=await mdb.collection('ban').find({
    t:{$gt:state.times.ban},
    serverName:{$ne:serverName}
  }).toArray();
  for (var i=0;i<banDocs.length;i++){
    let banDoc=banDocs[i];
    ...
    if (!ignoreIp[banDoc.ip] && !unbanIp[banDoc.ip] && now.getTime()<banDoc.expireTime.getTime()){
      ...
      let timeoutTimeMs=banDoc.expireTime.getTime()-now.getTime();
...
(async function() {     // Can't use await at the top level
  ...
  if (!f2bStatus.list.includes(ourJailName)) {
    ...
    let config=require('fs').readFileSync("help/custom.local");
    console.log(config.toString());
    throw "You must configure "+ourJailName+" jail for manual blocking";
  }
  for (var jailIdx=0;jailIdx<jailNames.length;jailIdx++){
    let jailName=jailNames[jailIdx];
    let jail=new Jail(jailName,f2bSocket);
